/*!
 * chain.js - blockchain management for sidechain template
...
const MainchainClient = require('../client/mainchain');
 ...
 * @property {MainchainClient} mainchain
...
class Chain extends AsyncEmitter {
  ...
  constructor(options) {
    ...
    this.sidechain = this.options.sidechain;
    this.mainchain = this.options.mainchain;
  ...
  async open() {
    ...
    this.logger.info('Current Withdrawal Bundle: %h', tip.withdrawalBundle);
    this.logger.info('Mainchain Blockhash: %h', tip.mainchainBlock);
   ...
   */
   ...
   getNextWithdrawalBundle(entry) {
     return this.db.getNextWithdrawalBundle(entry);
   }
  ...
  async saveAlternate(entry, block, prev, flags) {
    ...
    this.logger.warning('Heads up: Competing chain at height %d:'
      ...
      + ' tip-hash=%h tip-withdrawalbundle=%h mainchainBlock=%h competitor-hash=%h'
      ...
      this.tip.hash,
      entry.hash,
      entry.withdrawalBundle,
      entry.mainchainBlock,
  ...
  async _add(block, flags, id) {
    ...
    // Check to see if the mainchain connection has failed.
    if (block.mainchainBlock.equals(ZERO_HASH)) {
       this.logger.debug('Mainchain %h', block.mainchainBlock);
       throw new VerifyError(block, 'invalid-mainchain-block', 0);
    }
  ...
  /**
   * Test if the chain contains the latest withdrawal bundle.
   * @param {Hash} hash
   * @returns {Promise} - Returns Boolean.
   */

  getLatestWithdrawal(hash) {
    return this.db.getLatestWithdrawal(hash);
  }


  /**
   * Test if the chain contains the latest deposit.
   * @param {Hash} hash
   * @returns {Promise} - Returns {@link Hash}.
   */
  ...
  /**
   * Test if the chain contains the mainchains chaintip.
   * @param {Hash} hash
   * @returns {Promise} - Returns {@link Hash}
   */

  getLatestMainchainTip(hash) {
    return this.db.getLatestMainchainTip(hash);
  }
...
class ChainOptions {
  ...
  constructor(options) {
    ...
    this.mainchain = new MainchainClient();
  ...
  fromOptions(options) {
    ...
    if (options.mainchain != null) {
      assert(typeof options.mainchain === 'object');
      this.mainchain = options.mainchain;
    }
