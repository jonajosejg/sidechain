...
class ChainDB {
  ...
  async getLatestWithdrawal(hash) {
    ...
    const mainchainEntry = ChainEntry.fromRaw(raw);
  ...
  async _save(entry, block, view) {
    ...
    const withdrawalHash = block.withdrawalBundle;

    // Hash->height index.
    this.put(layout.h.encode(hash), fromU32(entry.height));

    // WithdrawalBundle Hash->hash
    this.put(layout.P.encode(withdrawalHash), fromU32(entry.height));
    ...
    // BMM entry
    this.bmmCache.set(entry.withdrawalBundle, entry);
