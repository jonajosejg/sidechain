...
class ChainDB {
  ...
  /**
   * Retrieve the chains latest withdrawal bundle hash.
   * @param {Hash} hash
   * @returns {Promise} - Returns {@link ChainEntry}.
   */

  async getLatestWithdrawalBundle(hash) {
    assert(Buffer.isBuffer(hash));

    if (hash.equals(consensus.ZERO_HASH))
      return null;

    const raw = await this.db.get(layout.P.encode(hash));
  ...
  /**
   * Get next withdrawal bundle.
   * @param {ChainEntry} entry
   * @returns {Promise} - Returns ChainEntry.
   */

   async getNextWithdrawalBundle(entry) {
     const next = await this.getEntryByHeight(entry.height + 1);

     if (next.mainchainBlock.equals(entry.withdrawalBundle))
       return null;

     return next.withdrawalBundle;
   }
  ...
  /**
   * Get the latest sidechain deposit by hash only
   * @returns {Hash} hash
   * @returns {Promise} - Returns {@link Hash}.
   */

  async getLatestDeposit(hash) {
    return this.db.get(layout.L.encode(hash));
  }

  /**
   * Get the latest mainchain tip by hash only
   * @returns {Hash} hash
   * @returns {Promise} - Returns {@link Hash}.
   */

  async getLatestMainchainTip(hash) {
    return this.db.get(layout.M.encode(hash));
  }
  ...
  async _save(entry, block, view) {
    ...
    const withdrawalBundle = block.withdrawalBundle;
    const mainchainBlock = block.mainchainBlock;
    ...
    // WithdrawalBundle->withdrawalbundle index.
    this.put(layout.P.encode(entry.withdrawalBundle), withdrawalBundle);
    this.cacheHash.push(entry.withdrawalBundle, withdrawalBundle);

    // MainchainBlock->hash index.
    this.put(layout.M.encode(entry.mainchainBlock), mainchainBlock);


    // BMM block entry->hash index.
    this.bmmCache.set(entry.withdrawalBundle, withdrawalBundle);
  ...
  async _reconnect(entry, block, view) {
    const hash = block.hash();
    const mainchainHash = block.mainchainBlock;
  ...
  async reset(block) {
    ...
    for (;;) {
      ...
      this.del(layout.W.encode(tip.withdrawalBundle));
  ...
  async connectBlock(entry, block, view) {
    ...
    // Check that the current block entries aren't a NULL_HASH
    if (entry.mainchainBlock === consensus.ZERO_HASH)
      throw new Error('Mainchain connection is not present.');

    // Update chain state value.
    // Meanwhile also checking for withdrawal bundles.
    for (let i = 0; i < block.txs.length; i++) {
      const tx = block.txs[i];
