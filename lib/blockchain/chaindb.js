...
const MainchainEntry = require('./mainchainentry');
const CoinEntry = require('../coins/coinentry');
const WithdrawalBundle = require('../sidechain/withdrawals');
...
class ChainDB {
  ...
  constructor(options) {
    ...
    this.withdrawals = new WithdrawalBundle();
   ...
   * @param {Hash} hash
   * @returns {Promise} - Returns {@link MainchainEntry}.
   */

  async getLatestWithdrawal(hash, index) {
    assert(Buffer.isBuffer(hash));

    if (hash.equals(consensus.ZERO_HASH))
      return null;

    const cache = this.bmmCache.get(hash);

    const raw = await this.db.get(layout.W.encode(hash, index));

    if (!raw)
      return null;

    const mainchainEntry = MainchainEntry.fromRaw(raw);

    this.bmmCache.set(mainchainEntry.withdrawalBundle);

    return mainchainEntry;
  }
  ...
  async getLatestDeposit(hash) {
    return this.db.get(layout.x.encode(hash));
  }
