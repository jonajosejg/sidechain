 ...
 *   P[hash] -> latest withdrawal bundle outpoint
 *   W[hash] -> Withdrawal Outpoint
 *   d[hash] -> Deposit Outpoint
 *   I -> input withdrawal bundle
 *   M[hash] -> Mainchain Blockhash
 *   V -> db version
 *   O -> chain options
 *   R -> tip hash
...
const layout = {
  ...
  F: bdb.key('F'),
  S: bdb.key('S'),
  B: bdb.key('B', ['hash256']), // WithdrawalBundle
  W: bdb.key('W', ['hash256']), // Withdrawal
  L: bdb.key('L', ['hash256']), // Deposit
  d: bdb.key('d', ['hash256']),
  I: bdb.key('I'),
  M: bdb.key('M', ['hash256']),
