/*!
 * node.js - http node client for sidechain template
...
class NodeClient extends Client {
  ...
  /**
   * Auth with mainchain node.
   * @returns {Promise}
   */

  async authMainchain() {
    await this.call('authmainchain', this.password);
  }
  ...
  /**
   * Retrieve the work score from BMM blocks.
   * @param {Hash} hash
   * @param {Number} score
   * @returns {Promise}
   */

  getHashScore(hash, score) {
    assert(typeof hash === 'string');
    assert((score >>> 0) === score);
    return this.get(`/workscore/${hash}/${score}`);
  }
  ...
  /**
   * send withdrawal tx to local node
   * @param {TX} tx
   * @returns {Promise}
   */

  broadcastWithdrawal(tx) {
    assert(typeof tx === 'string');
    return this.post('/broadcastwithdrawal', {tx});
  }
