...
class Miner extends EventEmitter {
  ...
  async _createBlock(tip, address) {
    ...
    const attempt = new BlockTemplate({
      prevBlock: tip.hash,
      withdrawalBundle: tip.withdrawalBundle,
      height: tip.height + 1,
      version: version,
      time: time,
      bits: target,
...
class MinerOptions {
  ...
  constructor(options) {
    ...
    this.version = -1;
    this.addresses = [];
    this.coinbaseFlags = Buffer.from('mined by sidechain', 'ascii');
    this.preverify = false;

    this.minWeight = policy.MIN_BLOCK_WEIGHT;
    this.maxWeight = policy.MAX_BLOCK_WEIGHT;
    this.minWorkScore = policy.MIN_MAINCHAIN_WORKSCORE;
    this.maxWithdrawalWeight = policy.MAX_WITHDRAWAL_WEIGHT;
    this.maxWithdrawalBundleWeight = policy.MAX_WITHDRAWAL_BUNDLE_WEIGHT;
  ...
  fromOptions(options) {
    ...
    if (options.minWorkScore != null) {
      assert((options.minWorkScore >>> 0) === options.minWorkScore);
      assert(options.minWorkScore >= policy.MIN_MAINCHAIN_WORKSCORE,
        'Minimum Mainchain work must greater than or equal to MIN_MAINCHAIN_WORKSCORE');
      this.minWorkScore = options.minWorkScore;
    }

    if (options.maxWithdrawalWeight != null) {
      assert((options.maxWithdrawalWeight >>> 0) === options.maxWithdrawalWeight);
      assert(options.maxWithdrawalWeight <= policy.MAX_WITHDRAWAL_WEIGHT,
        'Max withdrawal weight must be below MAX_WITHDRAWAL_WEIGHT');
      this.maxWithdrawalWeight = options.maxWithdrawalWeight;
    }

    if (options.maxWithdrawalBundleWeight != null) {
      assert((options.maxWithdrawalBundleWeight >>> 0) === options.maxWithdrawalBundleWeight);
      assert(options.maxWithdrawalBundleWeight <= policy.MAX_WITHDRAWAL_BUNDLE_WEIGHT,
        'Max withdrawal bundle weight must be below MAX_WITHDRAWAL_BUNDLE_WEIGHT');
      this.maxWithdrawalBundleWeight = options.maxWithdrawalBundleWeight;
    }
