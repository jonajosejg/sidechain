/*!
 * template.js - block template object for bitcoin sidechain
...
class BlockTemplate {
  ...
  constructor(options) {
    ...
    this.commitment = Buffer.alloc(64);
    this.left = DUMMY;
    this.right = DUMMY;
    this.items = [];
  ...
  fromOptions(options) {
    ...
    if (options.withdrawalBundle != null) {
  ...
  /**
   * Create a previous block commitment hash.
   * @returns {Buffer}
   */
  ...
  /**
   * Create a previous block commitment script.
   * @returns {Script}
   */

  getPreviousBlock() {
    return Script.fromPreviousBlockCommitment(main, side);
  }
  ...
  getProof(nonce1, nonce2, time, nonce) {
    ...
    const data = this.getHeader(root, time);
  ...
  commit(proof) {
    ...
    block.withdrawalBundle = this.withdrawalBundle;
    block.time = time;
...
class BlockProof {
  ...
  constructor(hash, root, withdrawalBundle, nonce1, nonce2, time, nonce) {
    this.hash = hash;
    this.root = root;
    this.withdrawalBundle = withdrawalBundle;
    this.mainchainBlock = mainchainBlock;
    this.nonce1 = nonce1;
    this.nonce2 = nonce2;
    this.time = time;
    this.nonce = nonce;
