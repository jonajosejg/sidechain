/*!
 * fullnode.js - full node for bcoin, testchain
...
class FullNode extends Node {
  ...
  constructor(options) {
    super('sidechain', 'sidechain.conf', 'debug.log', options);
    ...
    // MainchainClient needs access to the mainchain daemon.
    this.mainchain = new MainchainClient({
      network: this.network,
      port: this.network.mainchainPort,
  ...
  init() {
    ...
    this.mainchain.on('error', err => this.error(err))
    ...
    this.mainchain.on('connect', async (connection) => {
      try {
        await this.mainchain.isConnected();
      } catch (e) {
        this.error(e);
      }
