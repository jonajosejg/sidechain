...
class HTTP extends Server {
  ...
  constructor(options) {
    ...
    this.mainchain = this.node.mainchain;
  ...
  init() {
    ...
    this.on('listening', (address) => {
      this.logger.info('Mainchain Node HTTP server listening on %s (port=%d).',
         address.address, this.network.mainchainPort);
    });
...
class HTTPOptions {
  ...
  constructor(options) {
    ...
    this.host = '127.0.0.1';
    this.port = 8080;
    this.mainchainPort = this.network.mainchainPort;
    this.ssl = false;
    this.keyFile = null;
    this.certFile = null;
  ...
  fromOptions(options) {
    ...
    if (options.mainchainPort != null) {
      assert((options.mainchainPort & 0xffff) === options.mainchainPort,
         'Mainchain Port must be a number that matches original network port.');
      this.mainchainPort = options.mainchainPort;
    }
