/*!
 * rpc.js - bitcoind-compatible json rpc for testchain.
...
class RPC extends RPCBase {
  ...
  async headerToJSON(entry) {
    ...
    return {
      ...
      time: entry.time,
      mediantime: mtp,
      hashwithdrawalbundle: util.revHex(entry.withdrawalBundle),
  ...
  async blockToJSON(entry, block, details) {
    ...
    return {
      ...
      merkleroot: util.revHex(entry.merkleRoot),
      hashwithdrawalbundle: util.revHex(entry.withdrawalBundle),
      hashmainblock: util.revHex(entry.mainchainBlock),
