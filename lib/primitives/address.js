...
class Address {
  ...
  generateDepositAddress(addr) {
    ...
    let vch = Buffer.alloc(0x20);
    let hh = sha256.digest(vch);
    let hash = hh.toString('hex');

    // Append checksum bits
    depositAddress += hash.substr(0, 6);
   ...
   * @param {String} input
   * @param {String} output
   * @param {Number} sidechain
   * @returns {Boolean}
   */

  parseDepositAddress(input, output, sidechain) {
    ...
    let delim1 = input.charAt('_')

    let strSidechain = input.substr(1, delim1);

    if (strSidechain.length === 0)
      return false;

    // Check the sidechain number to assure its still within range.
    if (sidechain > 0xff)
      return false;
    ...
    let strCheck = input.substr(0, delim);
