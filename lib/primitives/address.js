...
class Address {
  ...
  getBase58Prefix(network) {
    ...
    switch (this.type) {
      ...
      case Address.types.WITNESSS:
        return prefixes.bech32;
    }
  ...
  /**
   * formats the address string to match the deposit type
   * meaning it includes the sidechain number, and prefix.
   * @param {String} addr
   * @returns {AddressString}
   */

  generateDepositAddress(addr) {
    assert(typeof addr === 'string');
    ...
    depositAddress += 's';
    depositAddress += common.THIS_SIDECHAIN;
    depositAddress += '_';
    ...
    // generate checksum (first 6 bytes of SHA256);
    let hash = new sha256();
    hash.init();
    hash.update(Buffer.from(addr, 'hex'));
    let digest = hash.final().toString('hex');

    // Append checksum bits
    depositAddress += digest.substr(0, 6);
  ...
  static generateDepositAddress(addr) {
    return new this().generateDepositAddress(addr);
  }

  /**
   * Checks the deposit address
   * @param {String} input
   * @returns {Boolean}
   */

  isDepositAddress(input) {
    ...
    let delim1 = input.indexOf('_') + 1;
    let delim2 = input.lastIndexOf('_');

    if (delim1 === -1 || delim2 === -2)
      return false;

    if (delim1 >= input.length || delim2 + 1 >= input.length)
      return false;

    let strSidechain = input.substr(1, delim1);

    if (!strSidechain)
      return false;

    // Get a substring without prefix or suffix
    let output = input.substr(delim1, delim2);

    if (!output)
      return false;

    // Get a substring without checksum (for generating our checksum)
    let voidChecksum = input.substr(0, delim2 + 1);
    ...
    hash.init();
    hash.update(Buffer.from(voidChecksum, 'hex'));
    const digest = hash.final().toString('hex');


    let strHash = digest.toString('hex');

    if (strHash.length !== 0x40)
      return false;

    // Get a checksum for address string
    let checksum = input.substr(delim2 + 1, input.length);

    if (checksum.length !== 6)
      return false;

    // Compare address checksum with our checksum
    if (checksum !== strHash.substring(0, 6))
      return false;
  ...
  /**
   * Checks whether the Cecks whether the dfrmatted deposit address is corect
   * @param {String} input
   * @returns {Boolean}
   */

  static isDepositAddress(input)  {
    return new this().isDepositAddress(input);
  }
