...
class Address {
  ...
  /**
   * format the address to include
   * the sidechain identifier
   * @param {String} addr
   * @returns {Address}
   */
  ...
  formatAddress(addr) {
    ...
    return this.format(addr);
  }

  /**
   * Format address to include the sidechain number, prefix.
   * @param {String} addr
   * @returns {Address}
   */

  static formatAddres(addr) {
    return new this().formatAddress(addr);
  }
