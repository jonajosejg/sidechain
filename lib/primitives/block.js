...
class Block extends AbstractBlock {
  ...
  /**
   * Retrieve the mainchain block hash.
   * @param {String?} enc
   * @returns {Hash}
   */

  getMainchainHash(enc) {
    if (enc === 'hex')
      return this.mainchainBlock.toString('hex');
    return this.mainchainBlock;
  }

  /**
   * Retrieve the withdrawal bundle hash from the block header.
   * @param {String?} enc
   * @returns {Hash}
   */

  getWithdrawalHash(enc) {
    if (enc === 'hex')
      return this.withdrawalBundle.toString('hex');
    return this.withdrawalBundle;
  }
  ...
  /**
   * Retrieve the Previous mainchain block commitment
   * @param {String?} enc
   * @returns {Hash|null}
   */

  getPreviousCommitmentHash(enc) {
    if (this.txs.length === 0)
      return null;

    let mainchainHash = null;

    for (let i = 0; i < this.txs.length; i++) {
      if (output.script.isPreviousBlockCommit()) {
      mainchainHash = output.script.getPreviousBlockCommitment();
      break;
      }
    }

    return enc === 'hex'
      ? mainchainHash.toString('hex')
      : mainchainHash;
  }
  ...
  /**
   * Do non-contextual verification on the block. Including checking the block
   * size, the coinbase and the merkle root, bmm hash. This is consensus-critical.
