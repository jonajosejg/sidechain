...
class Script {
  ...
  /**
   * Inject properties from a withdrawal refund request
   * @param {Buffer} hash
   * @param {String|Buffer} id
   * @returns {Script}
   */

  generateWithdrawalRefund(hash, id) {
  ...
  /**
   * Create a script that represents a withdrawal bundle failed
   * on the mainchain
   * @param {Buffer} hash
   * @param {String|Buffer} id
   * @returns {Script}
  ...
  static generateWithdrawalRefund(hash, id) {
    return new this().generateWithdrawalRefund(hash, id);
  }
 ...
 /**
  * Test whether the script withdrawal bundle failed
  * on the mainchain
  * @returns {Boolean}
  */

  isFailedWithdrawalCommitment() {
    return this.raw.length >= 38
      && this.raw[0] === opcodes.OP_RETURN
      && this.raw[1] === 0xfa
      && this.raw.readUInt32BE(2, true) === 0x86c689;
  }
