...
class Script {
  ...
  isSidechainObj() {
    return this.raw.length >= 5
       ...
       &&  this.raw.readUInt32BE(2, true) === 0xacdcf66f;
   }
  ...
  isPreviousBlockCommitment() {
    return this.raw.length >= 69
     && this.raw[0] === opcodes.OP_RETURN
     && this.raw.readUInt32BE(2, true) === 0xfd7ad1ef;
 }
  ...
  /**
   * Create a previous block commitment.
   * @param {Buffer} hash
   * @returns {Script}
   */
  ...
  fromPreviousBlockCommitment(mainchain, sidechain) {
    assert(Buffer.isBuffer(mainchain));
    assert(Buffer.isBuffer(sidechain));

    const bw = bio.write(68);
    bw.writeU32BE(0xfd7ad1ef);
    bw.writeHash(mainchain);
    bw.writeHash(sidechain);
  ...
  /**
   * Get the mainchain block in the commitment hash if present.
   * @returns {Buffer|null}
   */

  getMainchainBlockHash() {
    if (!this.isPreviousBlockCommitment())
      return null;
  ...
  /**
   * Get the sidechain block in the commitment hash if present.
   * @returns {Buffer|null}
   */

  getSidechainBlockHash() {
    if (!this.isPreviousBlockCommitment())
      return null;

    return this.raw.slice(38, this.raw.length);
  }
  ...
  failedWithdrawalBundle(hash) {
    const bw = bio.write(36);

    bw.writeU32BE(0xfa86c689);
    bw.writeHash(hash);
  ...
  static failedWithdrawalBundle(hash) {
    return new this().failedWithdrawalBundle(hash);
  }

  /**
   * Create a generic that represents
   * an sidechain object
   * @param {Buffer} hash
   * @returns {Script}
   */

  sidechainObject(hash) {
    const bw = bio.write(hash);

    bw.writeU32BE(0xacdcf66f);
  ...
  generateWithdrawalRefund(withdrawalID, sig) {
    const bw = bio.write(101);

    bw.writeU32BE(0xfcd2e546);
    bw.writeBytes(withdrawalID);
    bw.writeBytes(sig);
  ...
  static generateWithdrawalRefund(withdrawalID, sig) {
    return new this().generateWithdrawalRefund(withdrawalID, sig);
  }
  ...
  generateWithdrawalBundle(hash) {
    const bw = bio.write(36);

    bw.writeU32BE(0xfb5345de);
    bw.writeHash(hash);
  ...
  static generateWithdrawalBundle(hash) {
    return new this().generateWithdrawalBundle(hash);
  }
  ...
  isWithdrawalBundleSpentCommit() {
    return this.raw.length >= 37
      && this.raw[0] === opcodes.OP_RETURN
      && this.raw.readUInt32BE(2, true) === 0xfb5345de;
  }
  ...
  isWithdrawalBundleHashCommitment(){
    return this.raw.length >= 37
       && this.raw[0] === opcodes.OP_RETURN
       && this.raw.readUInt32BE(2, true) === 0xef5d15fe;
  }
  ...
  /**
   * Test whether the output script is a withdrawal refund
   * @returns {Boolean}
   */
  ...
  isWithdrawalRefund() {
    return this.raw.length >= 102
      && this.raw[0] === opcodes.OP_RETURN
      && this.raw.readUInt32BE(2, true) === 0xfcd2e546;
  }
   ...
   * that indicates the Withdrawal Bundle failed
   * on the mainchain.
   * @private
   * @param {Buffer} hash
   */

  fromFailedWithdrawalBundleCommitment(hash) {
    const bw = bio.write(36);
    bw.writeU32BE(0xfa86c689);
    bw.writeHash(hash);
  ...
  static fromFailedWithdrawalCommit(hash) {
    return new this().fromFailedWithdrawalBundleCommitment(hash);
  }

 /**
  * Test whether the script withdrawal bundle failed on the mainchain
  * @param {Buffer} withdrawalBundle
  * @returns {Boolean}
  */

  isFailedWithdrawalCommitment(hash) {
    return this.raw.length >= 37
      && this.raw[0] === opcodes.OP_RETURN
      && this.raw.readUInt32BE(2, true) === 0xfa86c689;
  }
