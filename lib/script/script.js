...
class Script {
  ...
  static isPreviousBlockCommitment(mainchain, sidechain) {
  ...
  fromWithdrawalRefundRequest(withdrawalID, signature) {
    assert(Buffer.isBuffer(withdrawalID));
    assert(Buffer.isBuffer(sig));

    const bytes = Buffer.concat([withdrawalID, signature]);

    this.raw = Buffer.allocUnsafe(buf.length + 5);
    this.raw[0] = opcodes.OP_RETURN;
    this.raw[1] = 0xfc;
    this.raw[2] = 0xd2;
    this.raw[3] = 0xe5;
    ...
    let sig = this.raw.slice(37, this.raw.length);

    this.code.length = 0;
    this.code.push(Opcode.fromPush(withdrawal));
    this.code.push(Opcode.fromPush(sig));
  ...
  static fromWithdrawalRefundRequest(withdrawalID, sig) {
    return new this().fromWithdrawalRefundRequest(withdrawalID, signature);
  }
  ...
  /**
   * Get the withdrawal bundle hash in
  ...
  /**
   * Test whether the output script is a
   * withdrawal bundle hash commitment.
   * @returns {Boolean}
   */
  ...
  getFailedWithdrawalCommitment() {
    if (!this.isFailedWithdrawalCommitment())
      return null;
