/*!
 * script.js - script interpreter for testchain
...
class Script {
  ...
  /**
   * Test whether the output script is a previous block commitment.
   * @returns {Boolean}
   */

  isPreviousBlockCommitment() {
    return this.raw.length === 69
      && this.raw[0] === opcodes.OP_RETURN
      && this.raw.readUInt32BE(1, true) === 0xfd7ad1ef;
  }
  ...
  /**
   * Encode withdrawal fees into an OP_RETURN.
   * @param {Number} amount - the amount to encode in fees.
   * @returns {Script}
   */

  encodeWithdrawalFees(amount) {
    assert(Number.isSafeInteger(amount), 'Amount must be an integer.');

    const U32 = Buffer.allocUnsafe(0x08);
    U32.writeUInt32LE(amount & 0xffffffff, 0);
    U32.writeUInt32LE(Math.floor(amount / 0x10000000) , 4);
  ...
  static encodeWithdrawalFees(amount) {
    return new this().encodeWithdrawalFees(amount);
  }
  ...
  /**
   * Inject properties from a prevmainchain block commitment.
   * @private
   * @param {Buffer} mainchain
   * @param {Buffer} sidechain
   * @returns {Script}
   */

  fromPreviousBlockCommitment(mainchain, sidechain) {
    assert(Buffer.isBuffer(mainchain) && mainchain.length === 32);
    assert(Buffer.isBuffer(sidechain) && sidechain.length === 32);

    const buf = Buffer.concat([mainchain, sidechain]);

    this.raw = Buffer.allocUnsafe(5 + buf.length);
    this.raw[0] = opcodes.OP_RETURN;
    this.raw[1] = 0xfd;
    this.raw[2] = 0x7a;
    this.raw[3] = 0xd1;
    ...
    mainchain.copy(this.raw, 5);

    mainchain = this.raw.slice(5, 37);
    sidechain = this.raw.slice(37, this.raw.length);

    this.code.length = 0
    this.code.push(Opcode.fromOp(opcodes.OP_RETURN));
    this.code.push(mainchain);
    this.code.push(sidechain);

    return this;
  }

  /**
   * Create a block commitment using the mainchain block hash.
   * @param {Buffer} mainchain
   * @param {Buffer} sidechain
   * @returns {Script}
   */

  static fromPreviousBlockCommitment(mainchain, sidechain) {
    return new this().fromPreviousBlockCommitment(mainchain, sidechain)
  }


  /**
   * Test whether the output script represents a previous block commitment
   * between the mainchain, sidechain
   * @returns {Boolean}
   */
  ...
  /**
   * Inject properties from a block version commitment
   * @param {Number} version
   * @returns {Script}
   */

  fromBlockVersionCommit(num) {
    this.raw = Buffer.allocUnsafe(0x09);
    this.raw[0] = opcodes.OP_RETURN;
    this.raw[1] = 0xa7;
    this.raw[2] = 0xe6;
    this.raw[3] = 0x7e;
    ...
    const data = ScriptNum.fromNumber(num);

    this.code.length = 0;
    this.code.push(Opcode.fromOp(opcodes.OP_RETURN));
    this.code.push(Opcode.fromRaw(data));

    return this;
  }
  /**
   * Test whether the output script represnets a block version commitment
   * @returns {Boolean}
   */

  isBlockVersionCommit() {
  ...
  /**
   * Inject properties from a withdrawal bundle that
   * failed on the mainchain
   * @param {Buffer} hash
   * @returns {Script}
   */

  fromFailedWithdrawalBundle(hash) {
    assert(Buffer.isBuffer(hash) && hash.length === 32);

    this.raw = Buffer.allocUnsafe(hash.length + 5);
    this.raw[0] = opcodes.OP_RETURN;
    this.raw[1] = 0xfa;
    this.raw[2] = 0x86;
    this.raw[3] = 0xc6;
    this.raw[4] = 0x89;
    hash.copy(this.raw, 5);

    hash = this.raw.slice(5, this.raw.length);
  ...
  /**
   * Create a script that represents
   * failed withdrawal bundle commitments
   * @param {Buffer} hash
   * @returns {Script}
   */

  static fromFailedWithdrawalBundle(hash) {
    return new this().fromFailedWithdrawalBundle(hash);
  }

  /**
   * Tests whether the output script is a failed withdrawal bundle commitment
   * @returns {Boolean}
   */

  isFailedWithdrawalBundle() {
    return this.raw.length === 37
      && this.raw[0] === opcodes.OP_RETURN
      && this.raw.readUInt32BE(1, true) === 0xfa86c689;
  }

  /**
   * Output that represents a sidechain object.
   * @param {Buffer} hash
   * @returns {Script}
   */

  fromSidechainObject(hash) {
  ...
  static fromSidechainObject(hash) {
    return new this().fromSidechainObject(hash);
  }

  /**
   * Test whether the output script contains a sidechain object
   * @returns {Boolean}
   */

  isSidechainObject(hash) {
    return this.raw[0] === opcodes.OP_RETURN
      && this.raw.readUInt32BE(1, true) === 0xacdcf66f;
  }

  /**
   * Inject properties from a script that
   * represents a withdrawal refund request
   * @param {Buffer} withdrawal
   * @param {Buffer} signature
   * @returns {Script}
   */

  fromWithdrawalRefundRequest(withdrawalID, sig) {
    assert(Buffer.isBuffer(withdrawalID));
    assert(Buffer.isBuffer(sig));

    const bytes = Buffer.concat([withdrawalID, sig]);

    this.raw = Buffer.allocUnsafe(bytes.length + 5);
    this.raw[0] = opcodes.OP_RETURN;
    this.raw[1] = 0xfc;
    this.raw[2] = 0xd2;
    this.raw[3] = 0xe5;
    ...
    let withdrawal = this.raw.slice(5, 37);
    let signature = this.raw.slice(37, this.raw.length);

    this.code.length = 0;
    this.code.push(Opcode.fromPush(withdrawal));
    this.code.push(Opcode.fromPush(signature));
  ...
  /**
   * Create a script that represents a withdrawal refund
   * @param {Buffer} withdrawalID
   * @param {Buffer} sig - 65 byte signature
   * @returns {Script}
   */

  static fromWithdrawalRefundRequest(withdrawalID, sig) {
    return new this().fromWithdrawalRefundRequest(withdrawalID, sig);
  }

  /**
   * Test whether the output script is withdrawal refund request
   * @returns {Boolean}
   */

  isWithdrawalRefundRequest() {
    return this.raw.length === 102
       && this.raw[0] === opcodes.OP_RETURN
       && this.raw.readUInt32BE(1, true) === 0xfcd2e546;
  }

  /**
   * Inject properties from a script that
   * represents a spent withdrawal bundle
   * @param {Buffer} hash
   */

  fromWithdrawalBundleSpentCommitment(hash) {
    assert(Buffer.isBuffer(hash) && hash.length === 0x20);

    this.raw = Buffer.allocUnsafe(5 + hash.length);
    this.raw[0] = opcodes.OP_RETURN;
    this.raw[1] = 0xfb;
    this.raw[2] = 0x53;
    this.raw[3] = 0x45;
  ...
  /**
   * Create a script that represents a withdrawal bundle
   * @param {Buffer} hash
   * @returns {Script}
   */

  static fromWithdrawalBundleSpentCommitment(hash) {
    return new this().fromWithdrawalBundleSpentCommitment(hash);
  }

  /**
   * Test whether the output script is a withdrawal
   * bundle spent commitment.
   * @returns {Boolean}
   */

  isWithdrawalBundleSpentCommit() {
    return this.raw.length === 37
      && this.raw[0] === opcodes.OP_RETURN
      && this.raw.readUInt32BE(1, true) === 0xfb5345de;
  }

  /**
   * Get the spent withdrawalbundle commit in the script if present
   * @returns {Buffer|null}
   */

  getSpentWithdrawalBundle() {
    if (!this.isWithdrawalBundleSpentCommit())
      return null;

    return this.raw.slice(5, 37);
  }

  /**
   * Inject properties from a script commitment
   * of the current withdrawal bundle hash
   * @param {Buffer} hash
   */

  fromWithdrawalBundleHashCommit(hash) {
    assert(Buffer.isBuffer(hash) && hash.length === 32);
  ...
  /**
   * Creates a a withdrawal bundle hash commitment
   * @param {Buffer} hash
   * @returns {Script}
   */

  static fromWithdrawalBundleHashCommit(hash) {
    return new this().fromWithdrawalBundleHashCommit(hash);
  }

  /**
   * Test whether the output script is withdrawal bundle hash commitment
   * @returns {Boolean}
   */

  isWithdrawalBundleHashCommitment() {
    return this.raw.length === 37
      && this.raw[0] === opcodes.OP_RETURN
      && this.raw.readUInt32BE(1, true) === 0xef5d1dfe;
  }

  /**
   * Get the withdrawal bundle commitment hash if present.
   * @returns {Buffer|null}
   */

  getWithdrawalBundleHashCommitment() {
    if (!this.isWithdrawalBundleHashCommitment())
      return null;
  ...
  /**
   * Get the failed withdrawal commitment hash if present.
   * @returns {Buffer|null}
   */

  getFailedWithdrawalCommitment() {
    if (!this.isFailedWithdrawalBundle())
      return null;
